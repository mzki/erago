...
// ParagraphList is list of paragraphs to exchange bundled paragraphs at once.
type ParagraphList struct {
	unknownFields []byte
	Paragraphs    []*Paragraph `protobuf:"bytes,1,rep,name=paragraphs,proto3" json:"paragraphs,omitempty"`
}

func (x *ParagraphList) Reset() {
	*x = ParagraphList{}
}

func (*ParagraphList) ProtoMessage() {}

func (x *ParagraphList) GetParagraphs() []*Paragraph {
	if x != nil {
		return x.Paragraphs
	}
	return nil
...
func (this *ParagraphList) EqualVT(that *ParagraphList) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
	}
	if len(this.Paragraphs) != len(that.Paragraphs) {
		return false
	}
	for i, vx := range this.Paragraphs {
		vy := that.Paragraphs[i]
		if p, q := vx, vy; p != q {
			if p == nil {
				p = &Paragraph{}
			}
...
func (this *ParagraphList) EqualMessageVT(thatMsg any) bool {
	that, ok := thatMsg.(*ParagraphList)
	if !ok {
		return false
	}
...
// MarshalProtoJSON marshals the ParagraphList message to JSON.
func (x *ParagraphList) MarshalProtoJSON(s *json.MarshalState) {
	if x == nil {
		s.WriteNil()
		return
	}
	s.WriteObjectStart()
	var wroteField bool
	if len(x.Paragraphs) > 0 || s.HasField("paragraphs") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("paragraphs")
		s.WriteArrayStart()
		var wroteElement bool
		for _, element := range x.Paragraphs {
			s.WriteMoreIf(&wroteElement)
			element.MarshalProtoJSON(s.WithField("paragraphs"))
		}
		s.WriteArrayEnd()
	}
	s.WriteObjectEnd()
...
// MarshalJSON marshals the ParagraphList to JSON.
func (x *ParagraphList) MarshalJSON() ([]byte, error) {
	return json.DefaultMarshalerConfig.Marshal(x)
}

// UnmarshalProtoJSON unmarshals the ParagraphList message from JSON.
func (x *ParagraphList) UnmarshalProtoJSON(s *json.UnmarshalState) {
	if s.ReadNil() {
		return
	}
	s.ReadObject(func(key string) {
		...
		case "paragraphs":
			...
			s.ReadArray(func() {
				if s.ReadNil() {
					x.Paragraphs = append(x.Paragraphs, nil)
					return
				}
				v := &Paragraph{}
				v.UnmarshalProtoJSON(s.WithField("paragraphs", false))
				if s.Err() != nil {
					return
				}
				x.Paragraphs = append(x.Paragraphs, v)
			})
		}
	})
}

// UnmarshalJSON unmarshals the ParagraphList from JSON.
func (x *ParagraphList) UnmarshalJSON(b []byte) error {
	return json.DefaultUnmarshalerConfig.Unmarshal(b, x)
}
...
func (m *ParagraphList) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *ParagraphList) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *ParagraphList) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Paragraphs) > 0 {
		for iNdEx := len(m.Paragraphs) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.Paragraphs[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *ParagraphList) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *ParagraphList) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		if wireType == 4 {
			return fmt.Errorf("proto: ParagraphList: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: ParagraphList: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
			...
			if err := m.Paragraphs[len(m.Paragraphs)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
